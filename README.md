...
**注意**：当前仓库只包含工具函数模块，并未包含可直接运行的主程序；如需完整运行流程，请使用你自己的驱动脚本或补充主程序文件。

## 代码结构
- `QCNN2D.py`：运行入口封装器，会尝试加载同目录下的 `QCNN2D.PY` 主程序（如需使用，请自行提供该文件并注意大小写）。
...
## 运行与启动
### 方式一：运行主程序
如果你有主程序文件 `QCNN2D.PY`（仓库当前未包含该文件），请将它放在同目录后运行（注意大小写）：
```bash
python QCNN2D.PY
```
